defmodule DHCPv6.Server do
  ...
  def process_message(state, message, _client_ip, _port) do
    case message.msg_type do
      1 -> handle_solicit(state, message)      # SOLICIT
      3 -> handle_request(state, message)      # REQUEST
      4 -> handle_confirm(state, message)      # CONFIRM
      5 -> handle_renew(state, message)        # RENEW
      6 -> handle_rebind(state, message)       # REBIND
      8 -> handle_release(state, message)      # RELEASE
      10 -> handle_information_request(state, message)  # INFORMATION-REQUEST
  ...
  def expire_leases(state) do
    ...
    new_leases = 
      active_leases
      |> Enum.group_by(fn {{duid, _iaid}, _lease} -> duid end, fn {{_duid, iaid}, lease} -> {iaid, lease} end)
  ...
  defp renew_addresses(state, duid, ia_na_options) do
    ...
    |> Enum.map(fn {iaid, t1, t2, requested_addrs} ->
      case Map.get(state.leases, duid) && Map.get(state.leases[duid], iaid) do
        nil -> assign_new_address(state, duid, iaid, t1, t2, requested_addrs)
        lease ->
          renewed_lease = %{lease | 
  ...
  defp maybe_add_rapid_commit(message, true) do
    option = Option.new(14, <<>>)  # RAPID_COMMIT
  ...
  defp add_dns_servers(message, servers) when length(servers) > 0 do
    ...
    option = Option.new(23, server_data)  # DNS_SERVERS
    %{message | options: [option | message.options]}
  end
  ...
  defp add_status_code_option(message, code, message_text) do
    ...
    option = Option.new(13, option_data)  # STATUS_CODE
    %{message | options: [option | message.options]}
  end
